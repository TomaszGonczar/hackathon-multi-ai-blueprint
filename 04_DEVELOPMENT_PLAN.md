...
| **P8 — Event operation** | T+0 to T+end | Team | Solution + evidence | Submission | `DESIGNED` — the cadence table is operational, not a check; entry: event evidence log shows each cadence row exercised (observer reports, amendment numbers, merge records) |
...
**Q1 branch — one team or five (blueprint §10 Q1; §14):** this phase's shape depends on an answer the plan does not assume. One five-person team → P4 builds one fleet with up to five workstreams (lanes `L1`–`L5`) and the ownership registry (P4.2) maps surfaces to five members. Five separate teams → P4 builds five coordinated mini-fleets sharing one `REPO`, the registry maps surfaces to teams, and P4.5's integration order becomes cross-team. Per §14, the ownership registry is written only after Q1 is answered at P0. No answer is invented here.
...
**Q2 branch — exact duration (blueprint §10 Q2; §14):** which cut-order rows apply is decided by the duration answered at P0, and this plan does not assume it. A short event drops observer automation entirely — blueprint §8.2 cut-priority-1 row: `OB` continuous mode off, E24–E26 unused, manual Git reads remain — while the E1–E18 flow and the E27–E29 failure paths stay intact. A longer event runs the cadence table above unchanged. See also the "Time running out" row in §11.
...
- **Q2 (exact duration)** — determines whether the cut order in blueprint §8.2 drops cut-priority-1 or cut-priority-2 items (the cut-order namespace, distinct from plan phases P0–P9). A short event drops observer automation entirely and keeps manual Git reads.
